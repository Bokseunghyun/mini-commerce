...
export default function handler(req, res) {
  // Vercel의 파일 기반 라우팅이면 req.query.id가 들어옴
  // (예전 vercel.json 라우팅 시도에서는 req.url 파싱이 필요했어서 fallback 유지)
  const id = Number(
    req?.query?.id ??
    (req?.url ? req.url.split('/').pop() : NaN)
  );

  if (!Number.isFinite(id)) {
    return res.status(400).json({ message: '잘못된 상품 ID', code: 'INVALID_PRODUCT_ID' });
  }

  const product = PRODUCTS.find(p => p.id === id);

  if (!product) {
    return res.status(404).json({ message: '상품 없음', code: 'PRODUCT_NOT_FOUND' });
  }

  // 로컬 Express(productDetail.js)와 동일하게 3/4는 의도적 500
  if (product.id === 3 || product.id === 4) {
    return res.status(500).json({ message: '상품 조회 실패 (의도적 장애)', code: 'PRODUCT_DETAIL_INTENTIONAL_ERROR' });
  }
