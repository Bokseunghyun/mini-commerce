...
모든 영속 상태(상품/계정/리뷰/장바구니/위시리스트/주문/쿠폰)는 **Postgres**에 저장됩니다. `DATABASE_URL` 미설정 시 모든 데이터 API가 **503 `DB_NOT_CONFIGURED`** 를 반환합니다. → 설정 방법은 **[DEPLOYMENT.md](DEPLOYMENT.md)** 참고
...
# 1) DB 연결 문자열 설정 (.env.example을 .env로 복사 후 값 입력, Neon 무료 DB 권장 - DEPLOYMENT.md)
...
> `npm run start-api`와 `npm run db:init`은 레포 루트의 **`.env` 파일을 자동으로 읽습니다** (`scripts/load-env.js`, 셸 환경변수가 있으면 그 값이 우선). 자세한 배포/로컬 설정은 [DEPLOYMENT.md](DEPLOYMENT.md) 참고.
...
- [DEPLOYMENT.md](DEPLOYMENT.md) - **Vercel + Neon 배포/운영 가이드** (DB 설정, 자동 리셋, 트러블슈팅)
