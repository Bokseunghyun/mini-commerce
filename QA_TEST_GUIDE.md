...
- 품절 상태는 `soldout-badge-{id}` 노출과 구매버튼 disabled로 검증합니다.
...
- 홈 카드의 하트 `wishlist-toggle-{id}` 클릭 → `aria-pressed` `false`→`true` 토글
...
- `[data-testid="soldout-badge-{id}"]` - 품절 뱃지 (재고를 0으로 낮추면 노출)
...
- 상품 카드/하트/품절 뱃지: `product-card-{id}`, `wishlist-toggle-{id}`, `soldout-badge-{id}` 등
